...
function replaceEmail(adresses, oldEmail, newEmail) {
    const index = adresses.indexOf(oldEmail);
    console.log(index);
    adresses.splice(index, 1, newEmail);
    return adresses;
}
const updatesAdresses = replaceEmail(emailadresses, 'm.vandergeest@novi-education.nl', 'm.vandergeest@novi.nl');
console.log(updatesAdresses);
